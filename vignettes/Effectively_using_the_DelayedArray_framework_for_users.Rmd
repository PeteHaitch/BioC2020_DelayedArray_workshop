...
This workshop will be a feature [introductory material](https://docs.google.com/presentation/d/1_v4IuKLFs781pp7x0BwUxv7mwCfPLBTWu320g3lQmME/edit?usp=sharing), 'live' coding, and Q&A.
...
- **TODO**: Check for additional packages
...
```{r}
# The TENxBrainData package loads and attaches the HDF5Array package, amongst 
# others.
...
With data from 1.3 million cells, this is roughly 100,000-times more samples than a typical bulk RNA-seq dataset and would occupy 146 GB in-memory if stored as an ordinary *matrix*.
...
```{r}
# We'll discuss the use of `withDimnames = FALSE` later in the workshop.
...
We can now clearly see that `tenx_counts` is no ordinary *matrix*. In fact, it is an *HDF5Matrix*, which is a type of *DelayedArray*[^4].
...
We can learn more about the internals of the `tenx_counts` object using the `seed()` function[^5].

[^5]: We will revisit the `chunkdim` component of the *HDF5Matrix* object later in the workshop in [Chunking]
...
Here the data at `r print(object.size(as.array(tenx_counts_subset)), units = "auto")` are small enough to load into memory, but what if that's not the case[^9]?
...
[^9]: Recall, For example, that the `tenx_counts` data would occupy \> 146 GB in memory.
...
[^12]: In the above example it's safe because `tenx_counts_subset_realized` only requires around 100 Mb of memory.
...
[^14]: We'll discuss why you might want this in the second half of the workshop.
