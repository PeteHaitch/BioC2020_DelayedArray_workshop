...
| First contact                                   | 25m  |
| Real world encounters                           | 15m  |
| Package ecosystem                               | 10m  |
| Workflow tips for DelayedArray-backed analyses  | 40m  |
...
Here, we will quantify expression on the log~2~ scale using a crude library size normalization.

```{r}
norm_log_expression <- sweep(
  log2(tenx_counts_subset + 1L), 
  MARGIN = 2, 
  STATS = log2(lib_sizes),
  FUN = "-")
norm_log_expression
```
...
median_expression <- rowMedians(norm_log_expression)
...
for (j in seq_len(ncol(tenx_counts_subset))) {
  ...
  lib_sizes_loop_over_cols[j] <- colSums(
    as.matrix(tenx_counts_subset[, j, drop = FALSE]))
}
# Check we get the same result as before.
...
for (j in seq_len(ncol(tenx_counts_subset))) {
  ...
  for (i in 1:2) {
    i1 <- (i - 1) * nr / 2 + 1
    i2 <- i * nr / 2
    tmp_colsums[i] <- colSums(as.matrix(tenx_counts_subset[seq(i1, i2), j]))
  }
...
```{r}
# Conceptually equivalent to rowSums(tenx_counts_subset)
...
- Advanced topic, skip over for the most part.

### Realization

- [ ] Implicit realization
- [ ] Explicit realization
- [ ] Realizing to a non-array backend.
  - [ ] Matrix multiplication

## Real world encounters with DelayedArray (15 mins)
...
## Workflow for DelayedArray-backed analyses (40 mins)
...
```{r}
bench::mark(
  colSums(as.matrix(tenx_counts_subset)),
