...
The biggest discrepency in performance is when performing the principal component analysis of the data.
However, by trading off additional computational time for reduced memory requirements, this is fully achievable and the results can be saved for downstream analysis.
...
Those with an interest in clever performance tricks may be interested in the *DeferredMatrix*, *LowRankMatrix*, and *ResidualMatrix* classes, derived from the *DelayedMatrix* class, that are implemented in the `r BiocStyle::Biocpkg("BiocSingular")` package.
