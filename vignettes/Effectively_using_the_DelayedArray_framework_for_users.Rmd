...
- Reason about parallelization strategies for algorithms operating on *DelayedArray* objects.
...
In my own research I have made extensive use of the DelayedArray framework when analysing whole genome bisulfite sequencing (WGBS) data sets.
...
This sized data is challenging, but well within the realms of a high performance computing available at a modern research institute.
However, the brain also has extensive non-CpG methylation and there are is an order of magnitude more non-CpG loci.
...
# NOTE: We'll turn off parallelisation for didactic purposes.
...
# NOTE: We'll discard cells with large mitochondrial proportions, using it as a
...
clusters <- quickCluster(
  ...
  #       clustering on that.
  #       TODO: Remove comment on timing.
  #       To run on the HDF5-backed data, simply use `x = sce` (this took 85 
  #       mins minutes on unix311 when stored in dense HDF5Matrix.
...
Under the hood, this involves computing column subs,  of the HDF5-backed count matrix.
...
Skipping over the details, we use functions from the `r BiocStyle::Biocpkg("scran")` package to decompose the variation in the log-expression matrix to a technical and biological component.
...
pcs <- denoisePCA(
  ...
  #       TODO: Remove comment on timing.
  #       This took 141 minutes on unix311 when stored in dense HDF5Matrix.
  # TODO: Remove the comment about time
...
> What it does exactly is (1) write all the assay data to an HDF5 file, and (2) serialize the object shell, which in this case is everything in the object that is not the assay data.
...
### Process, realize, repeat

When analysing large data sets, a workflow that is broken up into stages and saves the intermediate outputs can be help preserve ones sanity.
...
This means using `saveHDF5SummarizedExperiment()`/`loadHDF5SummarizedExperiment()`, or `realize()`-ing the result if there is sufficient memory available, after doing time consuming processing of the object.
...
For example, a normalized scRNA-seq data set carries around two matrices: the raw counts and the normalized expression values.
...
This comes to a chief advantage of using HDF5-backed *SummarizedExperiment* derivatives, namely that loading the saved data with `HDF5Array::loadHDF5SummarizedExperiment()` is really fast.
I made extensive use of this when processing large WGBS data sets as I could quickly load the *BSseq* object to compute summaries of the sample metadata (stored in the `colData` of the *BSseq* object), a process that used to take tens of minutes to hours because the 3 large matrices also had to be loaded into memory.
This has been so useful to me that I now keep even 'small' WGBS data sets as HDF5-backed *BSseq* objects.
...
By default, this is set to `r getAutoBlockSize()` meaning each block can use up to `r getAutoBlockSize()` / `1e6` = `r getAutoBlockSize() / 1e6` Mb of data.
...
Data stored in an HDF5 file are usually 'chunked' into sub-matrices (for matrix data) or hyper-cubes (for arrays of higher dimension)^[Chunking also applies to other storage formats, such as *RleArray* objects].
...
The difference is this: **block dimensions dictate how the data are accessed, chunk dimensions dictate how the data are stored**.
...
We'll demonstrate how chunking can affect performance by comparing computing the column sums of a column-chunked and row-chunked data set.

```{r}
x <- matrix(sample(1e8), ncol = 1e4, nrow = 1e4)
x_row <- writeHDF5Array(x, chunkdim = c(1, ncol(x)))
x_col <- writeHDF5Array(x, chunkdim = c(ncol(x), 1))
...
Look out for the `BPPARAM` argument in function documentation^[Unfortunately, simply typing the function name at the command prompt or using `args()` may not be sufficient to identify these functions. E.g., `args(denoisePCA)` returns `r args(denoisePCA)`, which doesn't include `BPPARAM` but `help("denoisePCA", "scran")` tells you this is an option (Footnote to the footnote: this is because `denoicePCA()` is an S4 generic with methods defined only for the first argument, `x`. The methods include the `BPPARAM` argument and are documented as such.)].

Fair warning: parallelization is tricky and performance of different strategies may not match expectations.

### What's in my HDF5 file?

I use this quite a bit: I've got an HDF5 file but I don't know (or have forgotten) what's in it.
...
Those with an interest in clever performance tricks may be interested in the *DeferredMatrix*, *LowRankMatrix*, and *ResidualMatrix* classes, derived from the *DelayedMatrix* class, which are implemented in the `r BiocStyle::Biocpkg("BiocSingular")` package.
...
object.size(y)
## 19523952 bytes
system.time(pc.out <- runPCA(y, 10, BSPARAM=IrlbaParam()))
##    user  system elapsed
##   5.146   0.240   5.386

# For comparison:
fit <- lm.fit(x=design, y=as.matrix(y0))
object.size(fit$residuals)
## 1200000392 bytes
system.time(pc.out <- runPCA(fit$residuals, 10, BSPARAM=IrlbaParam()))
##    user  system elapsed
## 105.118   0.244 105.377
````
...
Now comes the kicker: **don't use it if you can avoid it!**

Often times I find myself starting with a very large data set, do my initial processing using the disk-backed data, and run `saveHDF5SummarizedExperiment()` to produce a 'pristine' object.
I can then move it to a large-memory machine, do my computation (e.g., PCA), save the result, and **move on with my life**.

If you can load your data into memory and still compute on it you're always going to have a better time.
Analyses will be faster, simpler, and you will have more options available to you.
But when this isn't an option the DelayedArray framework is a powerful set of packages to help you get your work done.
...
A huge amount of gratitude is owed to Hervé Pagès, the developer of the `r BiocStyle::Biocpkg("DelayedArray")` and broader framework (and so much more!), and Aaron Lun, the main developer of the `r BiocStyle::Biocpkg("simpleSingleCell")` workflow and many of the key packages for analysing scRNA-seq data.
Thank you!
...
```{r}
# TODO: Try first converting to TENxMatrix and see what isSparse() tells me.
```
