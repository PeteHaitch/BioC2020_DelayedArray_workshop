...
| Real world encounters analysing scRNA-seq data  | 30m  |
| Workflow tips for DelayedArray-backed analyses  | 10m  |
...
## Real world encounter with DelayedArray analysing scRNA-seq dat (30 mins)
...
One of these outputs is an HDF5 file, which `DropletUtils::read10xCounts()` can read and return as a *TENxMatrix*, an HDF5-backed *DelayedMatrix*.
...
clusters <- quickCluster(
  ...
  #       clustering on that.
  #       To run on the HDF5-backed data, simply use `x = sce` (this took > 30 
  #       minutes on my laptop). 
  x = as(counts(sce, withDimnames = FALSE), "dgCMatrix"),
  use.ranks = FALSE, 
  BSPARAM = BiocSingular::IrlbaParam())
table(clusters)
# TODO: Stepping through quickCluster with the HDF5-backed data to figure out why 
#       it's so slow (running on unix311).
#       Right now, I can't figure it out. It worked fast when stepping through 
#       with DelayedArray:::set_verbose_processing(FALSE) but is slow with 
#       DelayedArray::set_verbose_processing(TRUE).
```
...
We can then perform a principal component analysis (PCA) of the log-expression matrix to create a reduced dimension representation of the data.
We also denoise the log-expression data by removing PCs corresponding to technical noise.

**TODO:** Reiterate that this works for HDF5-backed data^[Again, for the sake of time, we're going to cheat by loading the log-expression matrix into memory as a sparse matrix and computing the PCA using the in-memory data. But this functionality does work, albeit more slowly, for the HDF5-backed data.].
...
pcs <- denoisePCA(
  ...
  # TODO: Figure out where/why this takes so long with `x = sce`.
...
- standard tools works with HDF5-backed data
- Pain points
- Once you've got PCA you're home and hosed.

Regardless of whether the log-expression data are in-memory or on-disk, the PCs are stored in-memory as an ordinary matrix.
This means that downstream steps, such as computing the $t$-SNE co-ordinates and clustering, are fast because they use this 

which means that computing the $t$-SNE takes the same amount of time.s

#### t-SNE

We can now compute a $t$-SNE representation of the data and plot it.

```{r}
set.seed(1000)
sce <- runTSNE(sce, use_dimred = "PCA", perplexity = 30)
# TODO: Colour tSNE by an interesting gene.
plotTSNE(sce, colour_by = "CCL5")
```
...
## Workflow tips for DelayedArray-backed analyses (10 mins)
